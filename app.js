const KEY='gym_tycoon_v5';
const PREVKEY='gym_tycoon_v4';
...
function revenuePerVisitor(){let eq=machines.reduce((s,m)=>s+(g.machines[m[0]]||0)*m[4]*(1+((g.machineLv||{})[m[0]]||0)*.12),0);let mult=(g.staff.coach?1.15:1)*(g.staff.manager?1.10:1)*(g.facilities?.vending?1.08:1);return Math.round((13+eq*0.7+g.fee/55)*mult)}
...
function renderScene(){let scene=$('gymScene');scene.querySelectorAll('.equip,.memberDot,.facilitySpot').forEach(n=>n.remove());machineSlots().forEach((m,i)=>{let key=m.id+'_'+i,pos=(g.layout||{})[key],d=document.createElement('div');d.className='equip';d.style.left=(pos?.x??m.x)+'%';d.style.top=(pos?.y??m.y)+'%';d.innerHTML=`${m.icon}<small>${m.name}</small>`;let drag=false;const mv=e=>{if(!drag)return;e.preventDefault();let r=scene.getBoundingClientRect(),p=e.touches?e.touches[0]:e,x=Math.max(1,Math.min(90,(p.clientX-r.left)/r.width*100)),y=Math.max(5,Math.min(84,(p.clientY-r.top)/r.height*100));d.style.left=x+'%';d.style.top=y+'%';g.layout[key]={x,y}};const up=()=>{if(!drag)return;drag=false;document.removeEventListener('mousemove',mv);document.removeEventListener('mouseup',up);document.removeEventListener('touchmove',mv);document.removeEventListener('touchend',up);localStorage.setItem(KEY,JSON.stringify(g));toastMsg('器材位置已儲存')};d.addEventListener('mousedown',e=>{e.preventDefault();drag=true;document.addEventListener('mousemove',mv);document.addEventListener('mouseup',up)});d.addEventListener('touchstart',()=>{drag=true;document.addEventListener('touchmove',mv,{passive:false});document.addEventListener('touchend',up)},{passive:true});scene.appendChild(d)});let spots={shower:[6,18],vending:[7,36],lounge:[7,54],ptzone:[7,70]};facilities.forEach(f=>{if(!g.facilities?.[f[0]])return;let s=document.createElement('div');s.className='facilitySpot';s.style.left=spots[f[0]][0]+'%';s.style.top=spots[f[0]][1]+'%';s.innerHTML=`${f[2]}<small>${f[1]}</small>`;scene.appendChild(s)})}
...
function nextHour(){if(animating)return;let visitors=Math.min(demand(),capacity()),machineCap=Math.max(1,totalMachines()*2),servedCount=Math.min(visitors,machineCap),queueCount=Math.max(0,visitors-machineCap);let earn=Math.round(servedCount*revenuePerVisitor()*(queueCount>0?.94:1));g.money+=earn;g.todayIncome+=earn;g.totalRevenue+=earn;g.served+=servedCount;g.hour++;if(g.staff.coach)g.coachXP+=Math.round(servedCount*(g.facilities?.ptzone?1.35:1));if(Math.random()<.10&&g.vipMembers.length<12){let names=['阿哲','小美','Ken','Yuki','阿凱','Mina','Leo','小安','Rina','大雄'],goals=['增肌','減脂','體能','塑形'],n=names[Math.floor(Math.random()*names.length)];if(!g.vipMembers.some(v=>v.name===n)){let jobs=['上班族','學生','護理師','工程師','設計師','業務','老師','自由工作者'];g.vipMembers.push({name:n,goal:goals[Math.floor(Math.random()*goals.length)],job:jobs[Math.floor(Math.random()*jobs.length)],lv:1,progress:0});push(`🌟 ${n} 成為常客會員！`)}}g.vipMembers.forEach(v=>{v.progress+=Math.max(2,Math.round(servedCount/2));if(v.progress>=100){v.progress-=100;v.lv++;g.prestige+=2;push(`💪 ${v.name} 升到 Lv.${v.lv}`)}});
...
function buyMachine(id){let m=machines.find(x=>x[0]===id),count=g.machines[id]||0,cost=Math.round(m[3]*(1+count*.35));if(g.money<cost)return toastMsg('資金不足');g.money-=cost;g.machines[id]=count+1;g.rating=Math.min(5,g.rating+.04);push(`🏋️ 新增 ${m[1]}，場館容量提升。`);save();toastMsg(`${m[1]} 已進場`) }
...
function render(){ $('money').textContent=fmt(g.money);$('members').textContent=g.members;$('rating').textContent=g.rating.toFixed(1);$('prestige').textContent=g.prestige;$('day').textContent=g.day;$('clock').textContent=String(g.hour).padStart(2,'0')+':00';$('daybar').style.width=((g.hour-8)/14*100)+'%';$('incomeText').textContent=`今日收入 ${fmt(g.todayIncome)}`;$('hourlyText').textContent=fmt(hourly());$('levelText').textContent=`${g.level<3?'街角小型健身房':g.level<5?'人氣健身中心':'城市旗艦健身房'} · Lv.${g.level}`;$('sceneStatus').textContent=`容量 ${capacity()} 人 · ${totalMachines()} 台設備 · ${Object.keys(g.facilities||{}).filter(k=>g.facilities[k]).length} 項設施 · 月費 ${fmt(g.fee)}`;
 ...
 if($('leaguePanel')){let table=leagueTable(),rank=table.findIndex(x=>x.me)+1;$('leaguePanel').innerHTML=`<div class="card leagueHero"><div class="row"><div><small class="muted">健身房聯盟 · 第 ${g.leagueWeek} 週</small><h3 style="margin:4px 0">目前預測第 ${rank} 名</h3></div><div class="rankBadge">#${rank}</div></div><div class="muted">每 7 個營業日自動結算。排名越高，獎金與聲望越多。</div></div>`+table.map((r,i)=>`<div class="card rivalRow ${r.me?'me':''}"><b>${i+1}. ${r.icon} ${r.name}</b><span>${r.score} 分</span></div>`).join('')+`<div class="card" style="margin-top:8px"><div class="row"><span class="muted">聯盟冠軍次數</span><b>${g.rivalWins}</b></div></div>`;}
  $('feeSlider').value=g.fee;$('feeText').textContent=fmt(g.fee);
